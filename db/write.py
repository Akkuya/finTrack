...
def db_write_transaction(transaction: models.Transaction, db: sqlite3.Connection):
    ...
    db.execute(
        ...
        (transaction.date, transaction.name, transaction.amount,
         transaction.direction, transaction.account, transaction.currency,
...
def db_write_goal(goal: models.Goal, db: sqlite3.Connection):
    ...
    db.execute(
        "INSERT INTO GOALS(item_name, target_price, description, necessity, necessity_source, status, target_date) "
        "VALUES(?, ?, ?, ?, ?, ?, ?)",
        (goal.item_name, goal.target_price, goal.description,
         goal.necessity, goal.necessity_source, goal.status, goal.target_date),
    )
    db.commit()
